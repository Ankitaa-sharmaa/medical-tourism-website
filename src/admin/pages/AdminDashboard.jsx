...
import { MOCK_DOCTORS, MOCK_APPOINTMENTS, MOCK_TESTIMONIALS, MOCK_QUERIES } from '../data/mockData';
import AdminLayout from '../components/AdminLayout';
import {
  FaUserMd, FaCalendarAlt, FaStar, FaEnvelope, FaArrowRight,
} from 'react-icons/fa';
...
const StatCard = ({ icon, label, value, color, sub, to }) => (
  <Link to={to} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 group block">
    <div className="flex items-center justify-between mb-4">
      <div className={`w-12 h-12 ${color} rounded-2xl flex items-center justify-center text-white text-xl shadow-sm`}>
        {icon}
      </div>
...
const AdminDashboard = () => {
  const { items: doctors }      = useLocalData('admin_doctors',      MOCK_DOCTORS);
  const { items: appointments }  = useLocalData('admin_appointments', MOCK_APPOINTMENTS);
  ...
  const stats = useMemo(() => ({
    ...
    openQueries:   queries.filter(q => !q.status || q.status === 'new').length,
    pending:       appointments.filter(a => a.status === 'pending').length,
    approved:      appointments.filter(a => a.status === 'approved').length,
  }), [doctors, appointments, testimonials, queries]);
    ...
    <AdminLayout title="Dashboard">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <StatCard
          ...
          value={stats.doctors}
          color="bg-cyan-500"
          sub={`${doctors.filter(d => d.available).length} available`}
          to="/admin/doctors"
        />
        <StatCard
          icon={<FaCalendarAlt />}
            ...
            <div className="divide-y divide-slate-100">
              {recentAppts.map(a => (
                <div key={a.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-slate-50 transition">
                  <div className="w-8 h-8 rounded-full bg-violet-100 flex items-center justify-center text-violet-700 font-bold text-sm uppercase flex-shrink-0">
                    {a.name?.[0] ?? '?'}
                  </div>
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${STATUS_STYLE[a.status] ?? STATUS_STYLE.pending}`}>
                    {a.status ?? 'pending'}
                  </span>
                </div>
              ))}
            ...
            <div className="divide-y divide-slate-100">
              {recentQueries.map(q => (
                <div key={q.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-slate-50 transition">
                  <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-sm uppercase flex-shrink-0">
                    {q.name?.[0] ?? '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${STATUS_STYLE[q.status] ?? STATUS_STYLE.new}`}>
                    {q.status ?? 'new'}
                  </span>
                </div>
              ))}
      ...
      <div className="bg-gradient-to-br from-sky-50 to-cyan-50 border border-sky-200 rounded-2xl p-6">
        ...
        <div className="flex flex-wrap gap-3">
          {[
            { to: '/admin/doctors',      label: '+ Add Doctor',     cls: 'bg-cyan-500 hover:bg-cyan-400 text-white' },
            { to: '/admin/appointments', label: '+ Add Appointment', cls: 'bg-violet-500 hover:bg-violet-400 text-white' },
            { to: '/admin/testimonials', label: '+ Add Review',      cls: 'bg-amber-400 hover:bg-amber-300 text-white' },
            { to: '/admin/queries',      label: 'View Queries',      cls: 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-50' },
          ].map(({ to, label, cls }) => (
            <Link key={to} to={to} className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition shadow-sm ${cls}`}>
              {label}
            </Link>
          ))}
        </div>
